...
### Notifications
- `src/lib/notifications.ts` schedules local daily AM/PM reminders (`expo-notifications`, no push server) for whichever slots the active treatment requires. Called from onboarding's finish step and should be re-called (`rescheduleDailyReminders`) whenever the regimen changes.
- **Not yet implemented**: Yes/No/Skip notification action buttons and the reprompt scheduling that `computeRepromptTime` supports — currently only the initial reminder fires; logging still has to happen by opening the app.

### Screens/features not yet built
`Photos`, `Learn`, and `Me` tabs are intentionally stubbed with `src/components/coming-soon-screen.tsx` — not half-implemented, just not started. `Routine` shows only the current treatment + pause/resume; Start New Treatment and the Timeline are not built.
