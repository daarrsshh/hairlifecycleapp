...
- **Consistency shows charts, not scores. Both streak numbers were removed and should not come back.** A streak is binary, erased by one ordinary bad day, and says nothing about direction; "best streak ever" was worse still, since a current 2 against a personal best of 40 reads as a reminder of failure at exactly the moment someone opens the screen. The panels now answer, in order: am I getting better or worse (`WeeklyTrendChart`), what should I change (`TimeOfDayChart`), which item am I slipping on (per-item bars), and where were the gaps (heatmap). `computeCurrentStreak`/`computeBestStreak` still exist — Home's badge and the PDF export use them.
- `features/consistency/trend.ts` is pure and tested. Two rules in it are product decisions rather than implementation details: a week with nothing scheduled returns **`ratio: null`, never 0** (a paused week is a gap in the data, and a bar at the floor reads as failure), and the trend/insight copy only claims a direction when the change clears a threshold — under 5 points is noise, and manufacturing a story out of one missed dose is how a calm app turns into a nagging one. `TimeOfDayChart` hides itself below two distinct times for the same reason: one row is a stat, not an insight.
- `computeWeeklyTrend` is **bounded by construction** — a fixed week count, stopping at `earliestDate`. Same rule as `computeCurrentStreak`; see the bounded-loops section.
