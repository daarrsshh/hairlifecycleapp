...
- **Consistency shows three things, deliberately.** "Best streak ever" was removed rather than restyled: it isn't actionable, and it's most discouraging exactly when someone opens the screen — a current streak of 2 against a personal best of 40 reads as a reminder of failure, which is the dynamic the product exists to avoid. Don't reintroduce comparative or historical-best stats. What remains each answers a different question: how am I doing now (streak), which *specific* item am I slipping on (per-item bars), and where were the gaps (heatmap). `computeBestStreak` still exists and is still used by the PDF export.
