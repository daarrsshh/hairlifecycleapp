...
- **The Material Symbols font is preloaded once in the root layout** (`hooks/use-icon-font.ts`), and the app waits for it before rendering. On Android `SymbolView` renders `<Text style={{fontFamily}}>` with a glyph codepoint and each instance calls `loadAsync` in its own effect with its own `loaded` flag — five tab icons mounting together race on the same family, and an instance that flips `loaded` before the family is registered draws the missing-glyph box. The symptom was tab icons showing as ▯ until tapped. The hook returns `true` on *failure* as well as success, so a font that never arrives leaves missing icons rather than an app stuck on a blank screen.
